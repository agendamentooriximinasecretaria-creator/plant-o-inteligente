...
async function writeAudit(userId: string, usuarioNome: string, acao: string, modulo: string, detalhes: Record<string, unknown>, status = "sucesso") {
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "update_my_profile") {
      ...
      await writeAudit(caller.id, caller.email ?? "Usuário", "Perfil profissional atualizado", "perfil", { professional_id: professional.id });
      return json(200, { success: true });
    }

    const callerIsMaster = await isMaster(caller.id);
    if (!callerIsMaster) {
      return json(403, { error: "Apenas Gestor Master pode executar esta ação." });
    }
    ...
    if (action === "create_user") {
      ...
      await writeAudit(caller.id, caller.email ?? "Gestor Master", "Usuário criado", "usuarios", {
    ...
    if (action === "reset_password") {
      ...
      await writeAudit(caller.id, caller.email ?? "Gestor Master", "Senha redefinida por gestor", "usuarios", {
    ...
    if (action === "set_active") {
      ...
      await writeAudit(caller.id, caller.email ?? "Gestor Master", active ? "Usuário ativado" : "Usuário inativado", "usuarios", {
        target_user_id: userId,
        active,
      });
